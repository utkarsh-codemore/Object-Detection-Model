# -*- coding: utf-8 -*-
"""
Created on Sun Jun  4 11:03:00 2023

@author: Vikas
"""

import time, os, cv2, tensorflow as tf
import numpy as np
...
class Detector:
    ...
    def readClases(self, classesFilePath):
    ...
    def loadModel(self):
        ...
        "checkpoints", self.modelName, "saved_model"))
        print("Model " + self.modelName + " Loaded.")

    def predictImage(self, imagePath, threshold=0.5):
        image = cv2.imread(imagePath)
        bboxImage = self.createBox(image, threshold)
        
        cv2.imshow("Resukt", bboxImage)
        cv2.waitKey(0)
    ...
    def createBox(self, image, threshold=0.5):
        ...
        if len(bboxIdx) != 0:
            for i in bboxIdx:
                ...
                #####################################
                lineWidth = min(int((xmax - xmin) * 0.2), 
                                int((ymax - ymin) * 0.2))
    ...
    def predictVideo(self, videoPath, threshold = 0.5):
        ...
        while success:
            ...
            bboxImage = self.createBox(image, threshold)
            cv2.putText(bboxImage, "FPS: " + str(int(fps)), (20, 70), cv2.FONT_HERSHEY_PLAIN, 
                        2, (0,255,0), 2)
